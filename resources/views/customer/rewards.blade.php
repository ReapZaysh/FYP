    ...
    <div class="max-w-4xl mx-auto px-4 py-8 mb-20">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            @forelse($rewards as $reward)
                <div class="bg-white rounded-[2rem] shadow-sm border border-gray-100 overflow-hidden flex flex-col transition hover:shadow-md">
                    <div class="h-48 bg-gray-50 relative">
                        ...
                        @endif
                        <div class="absolute top-4 right-4 bg-white/90 backdrop-blur px-4 py-2 rounded-2xl shadow-sm text-amber-600 font-black flex items-center gap-1 border border-white">
                    ...
                    <div class="p-6 flex-grow flex flex-col">
                        <h3 class="font-black text-gray-900 text-xl mb-2">{{ $reward['name'] }}</h3>
                        <p class="text-gray-500 font-medium mb-6 flex-grow">{{ $reward['description'] }}</p>
                        ...
                        @auth
                            ...
                            @else
                                <div class="w-full bg-gray-100 text-gray-400 py-4 rounded-2xl text-lg font-black text-center cursor-not-allowed border border-gray-200">
                                    Need {{ number_format($reward['points_required'] - $userPoints) }} more points
                                </div>
                            @endif
                        @else
                            <a href="{{ route('customer.login', ['redirect' => route('customer.rewards')]) }}" class="w-full bg-blue-50 text-blue-600 hover:bg-blue-100 py-4 rounded-2xl text-lg font-black transition text-center block border border-blue-100">
                                Log in to Redeem
                            </a>
                        @endauth
                    </div>
            ...
            @empty
                <div class="col-span-full py-16 bg-white rounded-[2rem] border border-dashed border-gray-200 text-center">
                    ...
                    <h3 class="text-xl font-black text-gray-900 mb-2">Check back soon!</h3>
                    <p class="text-gray-500 font-medium">We are preparing some awesome rewards for you.</p>
                </div>
            @endforelse
        </div>
    </div>
