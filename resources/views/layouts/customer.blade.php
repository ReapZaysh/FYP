    ...
    <nav class="fixed bottom-0 w-full bg-white border-t border-gray-100 pb-safe z-50 shadow-[0_-4px_20px_-10px_rgba(0,0,0,0.1)]">
        <div class="max-w-7xl mx-auto px-4">
            <div class="flex justify-between items-center h-16">
                <!-- Menu (Default) -->
                <a href="{{ route('customer.menu', session('table_number')) }}" class="flex flex-col items-center justify-center w-full h-full space-y-1 {{ request()->routeIs('customer.menu') ? 'text-premium-brown' : 'text-gray-400 hover:text-gray-600' }}">
                ...
                <!-- Profile -->
                <a href="{{ auth()->check() ? route('customer.profile') : route('customer.login') }}" class="flex flex-col items-center justify-center w-full h-full space-y-1 {{ request()->routeIs('customer.profile') || request()->routeIs('customer.login') || request()->routeIs('customer.register') ? 'text-premium-brown' : 'text-gray-400 hover:text-gray-600' }}">
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
